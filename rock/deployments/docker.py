...
class DockerDeployment(AbstractDeployment):
    ...
    def _detect_gpu_count(self) -> int:
        """Detect number of host GPUs visible to Docker host."""
        try:
    ...
    def _resolve_round_robin_gpu_spec(self, gpu_count_per_sandbox: int) -> str | None:
        ...
        counter_path = "/tmp/rock_gpu_rr_counter"
    ...
    def _build_gpu_args(self) -> list[str]:
        ...
        Modes:
          - fixed: use gpu_device_request (or ROCK_GPU_DEVICE_REQUEST)
          - round_robin: assign devices per sandbox in round-robin
        """
    ...
    def _build_gpu_env_args(self) -> list[str]:
        """Inject CUDA/NVIDIA visibility env vars for deterministic GPU assignment."""
